...
```bash
pnpm add @zauth/core drizzle-orm @libsql/client
# or if you use better-sqlite3:
pnpm add @zauth/core drizzle-orm better-sqlite3
```
...
```ts
// auth.ts
import { createAuth, drizzleSQLiteAdapter } from "@zauth/core"
import { db } from "./db"
...
```ts
import { honoGuard, honoSessionMiddleware } from "@zauth/core"
import { auth } from "./auth"

app.use("*", honoSessionMiddleware(auth))
app.get("/me", honoGuard(), c => c.json({ user: c.get("user") }))
```
